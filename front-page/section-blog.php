<div class="front_page_section front_page_section_blog<?php
	$lingvico_scheme = lingvico_get_theme_option( 'front_page_blog_scheme' );
	if ( ! lingvico_is_inherit( $lingvico_scheme ) ) {
		echo ' scheme_' . esc_attr( $lingvico_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( lingvico_get_theme_option( 'front_page_blog_paddings' ) );
?>"
		<?php
		$lingvico_css      = '';
		$lingvico_bg_image = lingvico_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$lingvico_anchor_icon = lingvico_get_theme_option( 'front_page_blog_anchor_icon' );
	$lingvico_anchor_text = lingvico_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( lingvico_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$lingvico_css      = '';
			$lingvico_bg_mask  = lingvico_get_theme_option( 'front_page_blog_bg_mask' );
			$lingvico_bg_color_type = lingvico_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $lingvico_bg_color_type ) {
				$lingvico_bg_color = lingvico_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$lingvico_caption = lingvico_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $lingvico_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $lingvico_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( $lingvico_caption ); ?></h2>
				<?php
			}

			// Description (text)
			$lingvico_description = lingvico_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $lingvico_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $lingvico_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses_post( wpautop( $lingvico_description ) ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
				...
				if ( ! lingvico_exists_trx_addons() ) {
					...
					lingvico_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
				}
			}
			?>
			</div>
