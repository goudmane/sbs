...
if ( ! function_exists( 'lingvico_customizer_get_css' ) ) {

	function lingvico_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts = lingvico_customizer_add_theme_fonts( $fonts );
...
.top_panel .slider_engine_revo .slide_title, .blogger_news .post_item .label_sticky {
	{$fonts['h1_font-family']}
}
...
.widget.widget_categories ul li.cat-item, .widget_rss ul li a, .widget_rss ul li cite{
	{$fonts['h5_font-family']}
}
...
.masonry_wrap .post_item .post_info {
{$fonts['info_font-family']}
}

CSS;
			$rez          = apply_filters( 'lingvico_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			// Make theme-specific fonts rules
			$vars = lingvico_customizer_add_theme_vars( $vars );
			extract( $vars );
...
CSS;
			$rez          = apply_filters( 'lingvico_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars']  = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( lingvico_get_sorted_schemes() ) : array( $scheme );
			...
			if ( count( $schemes ) > 0 ) {
				...
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = lingvico_get_scheme_colors( $s );
					}
...
.search_mobile .search_form input[placeholder]::-webkit-input-placeholder,
.search_mobile .search_form .search_submit:before{
    color: {$colors['input_dark']};
}
...
.sticky .label_sticky, .sticky .post_label, .blogger_news .post_item .label_sticky {
	background-color: {$colors['text_link']};
	color: {$colors['extra_dark']};
}
...
.sc_layouts_title .post_info_counters .post_counters_item:hover, .breadcrumbs .breadcrumbs_item.current {
	color: {$colors['text_link']};
}
...
nav.menu_hover_fade_box .sc_layouts_menu_nav > li.current-menu-item > a,
nav.menu_hover_fade_box .sc_layouts_menu_nav > li.current-menu-parent > a,
nav.menu_hover_fade_box .sc_layouts_menu_nav > li.current-menu-ancestor > a{
     color: {$colors['extra_dark']}!important;
...
.lingvico_tabs .lingvico_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.mptt-navigation-tabs li.active a,
.lingvico_tabs .lingvico_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
.sheme_self.sidebar .lingvico_tabs .lingvico_tabs_titles li a {
	color: {$colors['alter_dark']};
	background-color: {$colors['alter_bg_hover']};
}
.sheme_self.sidebar div.esg-filter-wrapper .esg-filterbutton > span:hover,
.sheme_self.sidebar .mptt-navigation-tabs li a:hover,
.sheme_self.sidebar .lingvico_tabs .lingvico_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['alter_link']};
}
...
.sheme_self.sidebar .lingvico_tabs .lingvico_tabs_titles li.ui-state-active a {
...
.trx_addons_audio_wrap .trx_addons_audio_navigation > span span:hover{
    color: {$colors['bg_color']};
    background: {$colors['text_link']};
}
...
.format-audio .post_featured.without_thumb .post_audio .mejs-controls .mejs-button > button:hover,
.format-audio .post_featured.without_thumb .post_audio .mejs-controls .mejs-button > button:focus{
    color: {$colors['inverse_link']};
}

.format-audio .post_featured.without_thumb .post_audio .mejs-time, .wp-audio-shortcode .mejs-time{
    color: {$colors['inverse_link']};
}
...
.nav-links .page-numbers, .learn-press-pagination .page-numbers > li a,
...
.nav-links a.page-numbers:hover, .learn-press-pagination .page-numbers > li a:hover,
.nav-links .page-numbers.current, .learn-press-pagination .page-numbers.current {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
...
.post_item_single .post_header .post_meta_item.post_counters_comments:hover{
  color: {$colors['text_dark']};  
}
...
.comments_list_wrap .comment_info .comment_posted, .comments_list_wrap .comment_info .comment_author{
    color: {$colors['text_light']};
}
...
.sidebar .widget:not(.widget_popular_posts):not(.widget_search){
    background-color: {$colors['alter_bg_color']};
}
...
.extra_universal_post .widget .trx_addons_tabs .trx_addons_tabs_titles li a{
     background-color: {$colors['alter_bg_color']};
}
.extra_universal_post .widget.widget_popular_posts .trx_addons_tabs_content .post_item{
...
CSS;
					...
					$rez            = apply_filters(
						'lingvico_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'lingvico_filter_prepare_css', $css_str, $remove_spaces );
	}
}
