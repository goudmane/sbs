...
if ( ! function_exists( 'lingvico_need_page_title' ) ) {
	function lingvico_need_page_title() {
		return ! is_front_page() && apply_filters( 'lingvico_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'lingvico_show_layout' ) ) {
	function lingvico_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'lingvico_get_logo_image' ) ) {
	function lingvico_get_logo_image( $type = '' ) {
		$logo_image = '';
		if ( lingvico_is_on( lingvico_get_theme_option( 'logo_retina_enabled' ) ) && lingvico_get_retina_multiplier( 2 ) > 1 ) {
			$logo_image = lingvico_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' );
		}
		if ( empty( $logo_image ) ) {
			if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
				$logo_image = lingvico_get_theme_option( 'custom_logo' );
				if ( empty( $logo_image ) ) {
				...
				$logo_image = lingvico_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
			}
		}
		return lingvico_remove_protocol_from_url( $logo_image, false );
	}
}

// Return header video (if set)
if ( ! function_exists( 'lingvico_get_header_video' ) ) {
	function lingvico_get_header_video() {
		$video = '';
		if ( apply_filters( 'lingvico_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( lingvico_check_theme_option( 'header_video' ) ) {
				$video = lingvico_get_theme_option( 'header_video' );
...
// Show post banner
if ( ! function_exists( 'lingvico_show_post_banner' ) ) {
	function lingvico_show_post_banner( $banner_pos = '') {
		if ( is_singular( 'post' ) && '' !== $banner_pos ){
			$banner_code = lingvico_get_theme_option( $banner_pos . '_banner_code' );
			$banner_img = lingvico_get_theme_option( $banner_pos . '_banner_img' );
			$banner_class = !empty( $banner_img )
								? ( 'background' == $banner_pos ? '' : 'banner_with_image ' ) . lingvico_add_inline_css_class( 'background-image:url(' . esc_url( $banner_img ) . ')' )
								: '';
			$banner_link = lingvico_get_theme_option( $banner_pos . '_banner_link' );
			if ( ! empty( $banner_code ) || ! empty( $banner_img ) ) {
				...
				lingvico_show_layout( wp_kses_post( $banner_code ) );
...
if ( ! function_exists( 'lingvico_show_post_featured_image' ) ) {
	function lingvico_show_post_featured_image() {
		$seo = lingvico_is_on( lingvico_get_theme_option( 'seo_snippets' ) );
		// Featured image
		if ( lingvico_is_off( lingvico_get_theme_option( 'hide_featured_on_single' ) )
					&& ! lingvico_sc_layouts_showed( 'featured' )
					&& strpos( lingvico_get_post_content(), '[trx_widget_banner]' ) === false ) {
			do_action( 'lingvico_action_before_post_featured' );
			lingvico_show_post_featured( array( 'thumb_bg' => lingvico_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth' ) );
...
if ( ! function_exists( 'lingvico_show_post_title_and_meta' ) ) {
	function lingvico_show_post_title_and_meta() {

		$seo = lingvico_is_on( lingvico_get_theme_option( 'seo_snippets' ) );

		// Title and post meta
		if ( ( ! lingvico_sc_layouts_showed( 'title' ) || ! lingvico_sc_layouts_showed( 'postmeta' ) ) && ! in_array( get_post_format(), array( 'link', 'aside', 'status', 'quote' ) ) ) {
			do_action( 'lingvico_action_before_post_title' );
			$need_content_wrap = lingvico_get_theme_option( 'post_thumbnail_type' ) == 'fullwidth' && lingvico_get_theme_option( 'post_header_position' ) !== 'on_thumb';
			?>
			<div class="post_header post_header_single entry-header">
				...
				// Post meta
				$meta_components = lingvico_array_get_keys_by_value( lingvico_get_theme_option( 'meta_parts' ) );
				...
				if ( $show_categories && ! lingvico_sc_layouts_showed( 'postmeta' ) && lingvico_is_on( lingvico_get_theme_option( 'show_post_meta' ) ) ) {
					lingvico_show_post_meta(
						apply_filters(
							'lingvico_filter_post_meta_args',
							array(
				...
				// Post title
				if ( ! lingvico_sc_layouts_showed( 'title' ) ) {
					the_title( '<h1 class="post_title entry-title"' . ( $seo ? ' itemprop="headline"' : '' ) . '>', '</h1>' );
				}
				// Post subtitle
				$post_subtitle = lingvico_get_theme_option( 'post_subtitle' );
				if ( ! empty( $post_subtitle ) ) {
					?>
					<div class="post_subtitle">
						<?php lingvico_show_layout( $post_subtitle ); ?>
					</div>
					<?php
				}
				// Post meta
				if ( ! lingvico_sc_layouts_showed( 'postmeta' ) && lingvico_is_on( lingvico_get_theme_option( 'show_post_meta' ) ) ) {
					lingvico_show_post_meta(
						apply_filters(
							'lingvico_filter_post_meta_args',
							array(
			...
			<?php
			do_action( 'lingvico_action_after_post_title' );
		}
	}
}
...
if ( ! function_exists( 'lingvico_show_post_meta' ) ) {
	function lingvico_show_post_meta( $args = array() ) {
		if ( is_single() && lingvico_is_off( lingvico_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta<?php echo ! empty( $args['class'] ) ? ' ' . esc_attr( $args['class'] ) : ''; ?>">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					// Post categories
					$cats = get_post_type() == 'post' ? get_the_category_list( ' ' ) : apply_filters( 'lingvico_filter_get_post_categories', '' );
					if ( ! empty( $cats ) ) {
						?>
						<span class="post_meta_item post_categories"><?php lingvico_show_layout( $cats ); ?></span>
						<?php
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'lingvico_filter_get_post_date', lingvico_get_date() );
					if ( ! empty( $dt ) ) {
						?>
					...
					if ( $author_id > 0 ) {
						...
						$author_avatar = get_avatar( get_the_author_meta( 'user_email', $author_id ), 32 * lingvico_get_retina_multiplier() );
						?>
							<a class="post_meta_item post_author" rel="author" href="<?php echo esc_url( $author_link ); ?>">
								<?php
								if ( lingvico_get_theme_setting( 'show_author_avatar' ) ) {
									?>
									<span class="post_author_avatar">
										<?php lingvico_show_layout( $author_avatar ); ?>
									</span>
									<?php
								}
								?>
					...
					// Post counters
					$output = lingvico_get_post_counters( $args['counters'] );
					if ( '' != $output ) {
						lingvico_show_layout( $output );
					} elseif ( ! lingvico_exists_trx_addons() ) {
						if ( ! is_singular() || have_comments() || comments_open() ) {
							...
							?>
								<a href="<?php echo esc_url( get_comments_link() ); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><span class="post_counters_number">
									...
									<?php
									echo esc_html( _n( 'Comment', 'Comments', $post_comments, 'lingvico' ) );
									?>
									</span>
								</a>
							<?php
					...
					lingvico_show_share_links(
						array(
							...
							'caption' => 'drop' == $args['share_type'] ? esc_html__( 'Share', 'lingvico' ) : '',
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'lingvico' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );
				} else {
					// Custom counter
					do_action( 'lingvico_action_show_post_counter', $comp, get_the_ID() );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'lingvico_show_post_featured' ) ) {
	function lingvico_show_post_featured( $args = array() ) {
		$args = array_merge(
			array(
				'hover'         => lingvico_get_theme_option( 'image_hover' ), // Hover effect
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: lingvico_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? lingvico_get_no_image() : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = lingvico_get_post_image();
				if ( ! empty( $image ) ) {
					$image = lingvico_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . lingvico_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
		...
		if ( $args['singular'] ) {
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						lingvico_is_on( lingvico_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( lingvico_get_theme_setting( 'attachments_navigation' ) ) {
					?>
				...
				if ( has_post_thumbnail() && $args['seo'] ) {
					$lingvico_attr = lingvico_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $lingvico_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $lingvico_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb	|| ! empty( $args['show_no_image'] )
				...
				if ( $args['thumb_bg'] ) {
					...
					$output = lingvico_get_slider_layout( $slider_args );
					if ( '' != $output ) {
						lingvico_show_layout( $output );
					}
					//------- End: Moved down --------
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) ) ) {
					...
					if ( 'video' == $post_format ) {
						$video = lingvico_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = lingvico_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = lingvico_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = lingvico_make_video_autoplay( $video );
								?>
						...
						if ( empty( $audio ) ) {
							$audio = lingvico_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = lingvico_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								$media = urldecode( lingvico_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
								$media_author = '';
								$media_title  = '';
								if ( ! empty( $media ) ) {
									...
									// Parse tag params
									$media_author = strip_tags( lingvico_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
									$media_title  = strip_tags( lingvico_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );

								}
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php lingvico_show_layout( $media_author ); ?></div>
									<?php
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php lingvico_show_layout( $media_title ); ?></h5>
									<?php
								}
								// Display audio
								lingvico_show_layout( $audio );
								?>
							</div>
							<?php
						}
				...
				lingvico_show_layout( $post_info );
...
if ( ! function_exists( 'lingvico_get_no_image' ) ) {
	function lingvico_get_no_image( $no_image = '' ) {
		static $img = '';
		if ( empty( $img ) ) {
			$img = lingvico_get_theme_option( 'no_image' );
			if ( empty( $img ) ) {
				$img = lingvico_get_file_url( 'images/no-image.jpg' );
			}
		}
		if ( ! empty( $img ) ) {
			$no_image = $img;
...
if ( ! function_exists( 'lingvico_add_bg_in_post_nav' ) ) {
	function lingvico_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			...
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), lingvico_get_thumb_size( 'med' ) );
				$img = $img[0];
			} elseif ( lingvico_get_theme_setting( 'allow_no_image' ) ) {
				$img = $noimg;
			}
			if ( ! empty( $img ) ) {
		...
		if ( $next ) {
			...
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), lingvico_get_thumb_size( 'med' ) );
				$img = $img[0];
			} elseif ( lingvico_get_theme_setting( 'allow_no_image' ) ) {
				$img = $noimg;
			}
		...
		wp_add_inline_style( 'lingvico-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'lingvico_show_related_posts' ) ) {
	function lingvico_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : lingvico_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) lingvico_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			?>
			<section class="related_wrap">
				...
				if ( ! empty( $title ) ) {
					...
					esc_html_e( 'You May Also Like', 'lingvico' );
				}
				?></h3><?php
				if ( $args['slider'] ) {
					$slider_args                      = $args;
					...
					$slider_args['slides_space']      = lingvico_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = lingvico_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = lingvico_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'lingvico_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					lingvico_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						get_template_part( apply_filters( 'lingvico_filter_get_template_part', 'templates/related-posts', $style ), $style );
						?></div><?php
					}
				?></div><?php		
				if ( $args['slider'] ) {
					lingvico_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'lingvico_show_related_posts_callback' ) ) {
	add_action( 'lingvico_action_related_posts', 'lingvico_show_related_posts_callback' );
	function lingvico_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'lingvico_filter_show_related_posts', false ) ) {
			$lingvico_related_posts    = (int) lingvico_get_theme_option( 'related_posts' );
			if ( (int) lingvico_get_theme_option( 'show_related_posts' ) && $lingvico_related_posts > 0 ) {
				lingvico_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $lingvico_related_posts ) ),
						'columns'        => max( 1, min( 4, lingvico_get_theme_option( 'related_columns' ) ) ),
					),
					lingvico_get_theme_option( 'related_style' )
				);
			}
		}
	}
...
if ( ! function_exists( 'lingvico_show_portfolio_posts' ) ) {
	function lingvico_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? lingvico_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = lingvico_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = lingvico_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$lingvico_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the lingvico_ajax_get_posts action
if ( ! function_exists( 'lingvico_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_lingvico_ajax_get_posts', 'lingvico_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_lingvico_ajax_get_posts', 'lingvico_ajax_get_posts_callback' );
	function lingvico_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'lingvico' );
		}
...
// Show pagination
if ( ! function_exists( 'lingvico_show_pagination' ) ) {
	function lingvico_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = lingvico_get_theme_option( 'blog_pagination' );
		if ( 'pages' == $pagination ) {
			the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'lingvico' ),
					'next_text'          => esc_html__( '>', 'lingvico' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'lingvico' ) . ' </span>',
				)
			);
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load more posts', 'lingvico' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'lingvico' ) : esc_html__( 'Newest posts', 'lingvico' ) ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'lingvico' ) : esc_html__( 'Older posts', 'lingvico' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'lingvico_single_comments_field' ) ) {
	function lingvico_single_comments_field( $args ) {
