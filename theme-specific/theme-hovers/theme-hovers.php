...
if ( ! function_exists( 'lingvico_hovers_theme_setup3' ) ) {
	add_action( 'after_setup_theme', 'lingvico_hovers_theme_setup3', 3 );
	function lingvico_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		lingvico_storage_set_array_after(
			'options', 'border_radius', array(
				'button_hover' => array(
					'title'   => esc_html__( "Button's hover", 'lingvico' ),
					'desc'    => wp_kses_data( __( 'Select hover effect to decorate all theme buttons', 'lingvico' ) ),
					'std'     => 'default',
					'options' => array(
						'default'      => esc_html__( 'Fade', 'lingvico' ),
					),
					'type'    => 'select',
				),
				'image_hover'  => array(
					'title'    => esc_html__( "Image's hover", 'lingvico' ),
					'desc'     => wp_kses_data( __( 'Select hover effect to decorate all theme images', 'lingvico' ) ),
					'std'      => 'dots',
					'override' => array(
						'mode'    => 'page',
						'section' => esc_html__( 'Content', 'lingvico' ),
					),
					'options'  => lingvico_get_list_hovers(),
					'type'     => 'select',
				),
			)
		);
...
if ( ! function_exists( 'lingvico_hovers_theme_setup9' ) ) {
	add_action( 'after_setup_theme', 'lingvico_hovers_theme_setup9', 9 );
	function lingvico_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts', 'lingvico_hovers_frontend_scripts', 1010 );
		add_filter( 'lingvico_filter_localize_script', 'lingvico_hovers_localize_script' );
		add_filter( 'lingvico_filter_merge_scripts', 'lingvico_hovers_merge_scripts' );
		add_filter( 'lingvico_filter_merge_styles', 'lingvico_hovers_merge_styles' );
		add_filter( 'lingvico_filter_merge_styles_responsive', 'lingvico_hovers_merge_styles_responsive' );
		add_filter( 'lingvico_filter_get_css', 'lingvico_hovers_get_css', 10, 2 );
	}
}

// Enqueue hover styles and scripts
if ( ! function_exists( 'lingvico_hovers_frontend_scripts' ) ) {
	
	function lingvico_hovers_frontend_scripts() {
		if ( lingvico_is_on( lingvico_get_theme_option( 'debug_mode' ) ) ) {
			$lingvico_url = lingvico_get_file_url( 'theme-specific/theme-hovers/theme-hovers.js' );
			if ( '' != $lingvico_url ) {
				wp_enqueue_script( 'lingvico-hovers', $lingvico_url, array( 'jquery' ), null, true );
			}
		}
	}
}
...
if ( ! function_exists( 'lingvico_hovers_merge_styles' ) ) {
	
	function lingvico_hovers_merge_styles( $list ) {
...
if ( ! function_exists( 'lingvico_hovers_merge_styles_responsive' ) ) {
	
	function lingvico_hovers_merge_styles_responsive( $list ) {
...
if ( ! function_exists( 'lingvico_hovers_localize_script' ) ) {
	
	function lingvico_hovers_localize_script( $arr ) {
		$arr['button_hover'] = lingvico_get_theme_option( 'button_hover' );
		return $arr;
	}
}

// Merge hover effects to the single js
if ( ! function_exists( 'lingvico_hovers_merge_scripts' ) ) {
	
	function lingvico_hovers_merge_scripts( $list ) {
...
if ( ! function_exists( 'lingvico_hovers_add_icons' ) ) {
	function lingvico_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php lingvico_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'lingvico' ) : esc_html__( 'Buy now', 'lingvico' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'lingvico' );
					} else {
						esc_html_e( 'Details', 'lingvico' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php lingvico_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php lingvico_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
			...
			<div class="post_info">
				<div class="post_info_back">
					...
					if ( ! empty( $post_link ) ) {
						?>
							<a href="<?php echo esc_url( $post_link ); ?>" <?php lingvico_show_layout($target); ?>>
							<?php
					}
						the_title();
					if ( ! empty( $post_link ) ) {
			...
			// Hover style empty
			?>
			<a href="<?php echo esc_url( $post_link ); ?>" <?php lingvico_show_layout($target); ?> aria-hidden="true" class="icons"></a>
			<?php
		}
	}
}
...
// Add styles into CSS
if ( ! function_exists( 'lingvico_hovers_get_css' ) ) {
	
	function lingvico_hovers_get_css( $css, $args ) {
