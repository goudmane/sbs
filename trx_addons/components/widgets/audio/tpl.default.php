		...
		foreach ( $media as $item ) {
			...
			?>
                ...
                <div class="trx_addons_audio_player_wrap">
                    <?php
                    if ( ! empty( $item['cover'] ) ) {
                        $item['cover'] = trx_addons_get_attachment_url($item['cover'], apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('audio'), 'audio-default'));
                        ?>
                        <div class="sc_audio_item_image"><img src="<?php echo esc_url( $item['cover']) ?>" alt="<?php esc_attr_e('img', 'lingvico'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div>
                        <?php
                    }
                    ?>
                    <div class="trx_addons_audio_content_wraper">
                        <div class="trx_addons_audio_header">
                        <?php
                        if ( ! empty( $item['author'] ) || ! empty( $item['caption'] ) ) {
                            ...
                            <div class="audio_info">
                                ...
                                if ( '#' !== $now_text && count( $media ) > 1 ) {
                                    echo '<h5 class="audio_now_playing">' . ( ! empty( $now_text ) ? esc_html( $now_text ) : esc_html__( 'Now Playing', 'lingvico' ) ) . ': </h5>';
                                }
