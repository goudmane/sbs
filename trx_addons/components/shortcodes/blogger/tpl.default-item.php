...
?><div <?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) . (empty($post_link) ? ' no_links' : '') ); ?>><?php
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
									...
									apply_filters('trx_addons_filter_args_featured', array(
														...
														'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size($args['columns'] > 2 ? 'learnpress' : 'big'), 'blogger-default')
														), 'blogger-default')
								);
...
if($args['post_type'] === 'lp_course') {
    ...
    <div class="course-price">
        <?php if ($course->has_sale_price()) { ?>
            <span class="origin-price"> <?php lingvico_show_layout($course->get_origin_price_html()); ?></span>
        <?php } ?>
        <span class="price"><?php lingvico_show_layout($price); ?></span>
    </div>
    <?php
}
		...
		if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) {
            ...
            <div class="blogger_post_footer">
                <?php
                     if($args['post_type'] === 'lp_course') {
                ?>
                <div class="blogger_course_duration">
                    <?php
                        lingvico_show_layout($duration = $course->get_data( 'duration' ));
                    ?>
                </div>
                <?php
                 }
                ...
                if ( ! is_singular() || have_comments() || comments_open() ) {
                    ...
                    ?>
                    <div class="post_counters_item post_counters_comments icon-comment-light"><span class="post_counters_number">
                                        <?php
