	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="lingvico_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.lingvico_post_editor' );
					if ( link.length > 0 ) {
						lingvico_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.lingvico_post_editor', function(e) {
					lingvico_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('lingvico_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function lingvico_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
